...
class Offboard(object):
    ...
    def navigate_wait(self,
        ...
        while not rospy.is_shutdown():
            ...
            #     break
            if (t.x**2 + t.y**2 + t.z**2)**0.5 < tolerance:
                break
    ...
    def target_achieved(self, tolerance=0.2):
        ...
        t = self.__get_telemetry(frame_id='navigate_target')
        # print((t.x**2 + t.y**2 + t.z**2)**0.5)
        # return hypot(t.x, t.y, t.z) <= tolerance
        return (t.x**2 + t.y**2 + t.z**2)**0.5 <= tolerance


    def takeoff(self) -> None:
        print('Takeoff...', end='')
        self.navigate_wait(frame_id='body', speed=0.25, auto_arm=True)
        print('Done')

    def nonblock_takeoff(self) -> None:
        print('Takeoff. [non block]')
        self.navigate(frame_id='body', speed=0.25, auto_arm=True)
...
class FlightToPoint(IState):
    ...
    def initialise(self):
        self.machine.offboard.navigate(x=self.__x, y=self.__y, z=self.__z, frame_id='aruco_map', speed=0.25)
        return super().initialise()
...
class HoldCurrentPosition(IState):
    ...
    def initialise(self):
        ...
        self.machine.offboard.navigate(x=pos.x, y=pos.y, z=pos.z, frame_id='aruco_map', speed=0.25)
...
class VideoRecorder(object):
    def __init__(self):
        # Создание объекта CvBridge для преобразования ROS-изображений в OpenCV-формат
        self.bridge = CvBridge()

        # Подписка на топик с изображениями с камеры
        self.image_sub = rospy.Subscriber('/main_camera/image_raw', Image, self.image_callback)
        self.image_sub_termo_raw = rospy.Subscriber('/main_camera/image_raw', Image, self.image_callback)
        self.image_sub_termo_jet = rospy.Subscriber('/main_camera/image_raw', Image, self.image_callback)

        # Переменные для записи видео
        self.video_writer = None
        self.frame_size = None  # Размеры кадра
        self.start_time = None  # Время начала записи
        self.is_recording = False  # Флаг, указывающий, идет ли запись

        # Путь для сохранения видео
        self.save_path = rospy.get_param('~save_path', '/home/pi')
    ...
    def start_recording(self):
        ...
        if self.is_recording:
            rospy.logwarn("Запись уже запущена.")
            return

        if self.frame_size is None:
            rospy.logwarn("Размер кадра не определен. Ожидайте получения первого кадра.")
            return

        # Генерация имени файла
        self.start_time = time.strftime("%Y%m%d_%H%M%S", time.localtime())
        video_filename = f"main_video_{self.start_time}.avi"
        video_filename = f"termo_raw_video_{self.start_time}.avi"
        video_filename = f"termo_jet_video_{self.start_time}.avi"

        video_filepath = os.path.join(self.save_path, video_filename)

        # Инициализация записи видео
        fourcc = cv2.VideoWriter_fourcc(*'XVID')
        self.video_writer = cv2.VideoWriter(video_filepath, fourcc, 30.0, self.frame_size)
        self.is_recording = True
        rospy.loginfo(f"Начало записи видео: {video_filepath}")
    ...
    def stop_recording(self):
        ...
        if not self.is_recording:
            rospy.logwarn("Запись уже остановлена.")
            return
        ...
        if self.video_writer is not None:
            ...
            rospy.loginfo("Запись видео завершена.")

    def image_callback(self, msg):
        try:
            # Преобразование ROS-изображения в OpenCV-формат
            cv_image = self.bridge.imgmsg_to_cv2(msg, "bgr8")
        except Exception as e:
            rospy.logerr(f"Ошибка при преобразовании изображения: {e}")
            return

        # Получение размера кадра (если еще не получено)
        if self.frame_size is None:
            self.frame_size = (cv_image.shape[1], cv_image.shape[0])  # Ширина и высота
            rospy.loginfo(f"Инициализирован размер кадра: {self.frame_size}")

        # Запись кадра в видеофайл (если запись активна)
...
class MissionStateMachine(object):
    ...
    def __init__(self,
        ...
        self.__land_service = rospy.Service('/mission_land', Trigger, self.__land_response)
        ...
        self.__current_state = Takeoff(self).after_execution(lambda wh: wh.start_recording(), self.video_recorder).next(WaitFor(1.0)\
            .after_execution(lambda vh: vh.enable(), self.video_handler).next(
            MapBypass(self).after_execution(lambda vh: vh.disable(), self.video_handler).next(
            FlightToPoint(self, 6, 0.5).next(
            FlightToPoint(self, 6, 0.5, self.flight_height/2).after_execution(lambda wh: wh.stop_recording(), self.video_recorder).next(
            Land(self)
        ))))).initialise()
    
    
        # self.__current_state = Takeoff(self).next(
        #     FlightToPoint(self, 1, 1).next(WaitFor(2.0).next(FlightToPoint(self, 2, 0).next(WaitFor(2.0).next(FlightToPoint(self, 3, 1).next(FlightToPoint(self, 3, 1, self.flight_height/2).next(Land(self)
        # ))))))).initialise()
    ...
    def __land_response(self, request):
        if self.__disarming:
            return TriggerResponse(
                success=False,
                message="Reason: disarming"
        ...
        self.__mutex.acquire()
        self.__current_state = WaitFor(2.0).next(Land(self)).initialise()
        self.__returning_to_home = False
        self.__pause = False
        self.__landing = True
        self.__status = self.MissionStatus.LANDING.value
        self.__mutex.release()
